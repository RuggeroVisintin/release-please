...
export class FeatureFlagPlugin extends ManifestPlugin {
            ...
            try {
                ...
                if (output.trim()) {
                    ...
                    for (const line of lines) {
                        ...
                        console.log(
                            `[FeatureFlagPlugin] Found historical commit ${sha.substring(0, 7)} for ${flag}`
                        );
                    }
                }
            } catch (error) {
    ...
    private shouldIncludeCommit(commit: Commit): boolean {
        ...
        console.log(
            `[FeatureFlagPlugin] Commit ${commit.sha?.substring(0, 7)}: Feature-Flag=${flag}, enabled=${isEnabled}`
        );
